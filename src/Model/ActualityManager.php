...
class ActualityManager extends AbstractManager
{
    ...
    {
        $statement = $this->pdo->prepare("INSERT INTO " . self::TABLE . " (title, date, description, image)
    ...
    {
        $statement = $this->pdo->prepare("SELECT id, title, date, description, image FROM " . self::TABLE . " AS a
            ORDER BY a.id DESC
            LIMIT :limit");
